#!/usr/bin/env node
// #279 — ONLINE drift-check: porovná uložený seed `src/lib/server/cennik-pergola.json`
// so ŽIVÝM montalu.sk. Upozorní, keď montalu.sk zmení cenník (seed treba regenerovať
// cez `scripts/konfigurator-cennik-fetch.mjs`). Read-only (číta len cenový endpoint,
// žiadny submit/objednávka). Politický delay medzi volaniami.
//
// NIKDY v CI (externá sieť). Spustenie: `node scripts/konfigurator-cennik-drift.mjs`
...
const seed = JSON.parse(
	fs.readFileSync(path.join(root, 'src', 'lib', 'server', 'cennik-pergola.json'), 'utf8')
);
...
async function ceny(ctx, roofingSlug, length, width, modelKod) {
	const fd = new FormData();
	fd.append('_token', ctx.token);
	fd.append('valid_from', ctx.validFrom);
	fd.append('variants', 'PRLPC00040');
...
// vzorka: rohy + pár vnútorných buniek per (roofing, model)
...
async function main() {
	...
	let drift = 0;
	let checked = 0;
	for (const [vyplnKluc, slug] of Object.entries(seed.vyplne)) {
		for (const [modelKluc, modelKod] of Object.entries(seed.modely)) {
			const rows = seed.cennik[vyplnKluc]?.[modelKluc];
			if (!rows) continue;
			for (const [dK, wK] of vzorka(rows)) {
				const stored = rows[dK][wK];
				const live = await ceny(ctx, slug, Number(dK), Number(wK), modelKod);
				checked++;
				const bad =
				...
				if (bad) {
					drift++;
					console.log(
						`DRIFT ${vyplnKluc} ${modelKluc} ${dK}×${wK}: seed [${stored}] vs live ${live ? `[${live.mo},${live.vo}]` : 'nedostupné'}`
					);
				}
	...
	if (drift > 0) {
		console.log(
			'→ montalu.sk zmenil cenník. Regeneruj seed: node scripts/konfigurator-cennik-fetch.mjs'
		);
		process.exit(1);
	}
