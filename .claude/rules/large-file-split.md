...
| `src/lib/server/migracie.ts` | ~993 | AT CAP — každá nová migrácia extrahuj blok do migracie-seed (viď `migrations.md`) |
...
| `src/app.css` | ~879 | `@media print` split DONE (#376 stage 3 → `src/print.css`). #376 redizajn KOMPLETNÝ (stage 4 = tokenizácia, line-neutrálna) — `nav.*` extrakcia NEBOLA potrebná; pri ďalšom raste extrahuj `nav.*` do vlastného `.css` importovaného za app.css |
