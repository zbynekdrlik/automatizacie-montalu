...
## Kolo 13 — #277 PDF ponuka bez cien + kontaktný formulár (PR #283, v0.24.22, merge a19da0c → clean PR #284 e7ba5b9), ZATVORENÝ

- **Integrácia build-only lane:** `worktree-agent-af64200a08014a0fd` (server+komponent+PDF+v25) zmergnutá do `dev`, napojená na verejnú route `/konfigurator` (#275). Bump 0.24.21-dev.1 → merge build vetvy (auto, prekryv len CLAUDE.md router + package.json) → wiring.
- **Wiring:** `+page.server.ts` mountuje akciu `dopyt: dopytAction`; `+page.svelte` mapuje súhrn enginu → `PonukaConfig` (sklon/svetlá výška/plocha do `popis`) a vykreslí `DopytForm` v kroku súhrnu. Download-first: server vráti PDF base64, komponent spustí stiahnutie.
- **SvelteKit pasca (stála 1 CI cyklus, `cc4bcf8`):** pridanie pomenovanej `dopyt` vedľa `default` rozbilo VŠETKY kalkulačkové POSTy (SvelteKit nedovolí default + pomenované naraz). Oprava: kalkulačka je teraz pomenovaná `vypocet` (`action="?/vypocet"`). Nové pravidlo `.claude/rules/sveltekit-actions.md`. `svelte-check` to nezachytí, CI E2E áno.
- **Mutačný gate pasca (`9911139`):** diff-scoped gate mutuje CELÉ zmenené súbory → v25 do 45 KB `migracie.ts` = 547 mutantov (pokrytý väčšinou DB testov, 67 timeoutov), shard 2 padol na 20-min strope pri 41 %. Whole-file mutáciu jedného veľkého atomického súboru sharding nerozdelí. Oprava (skill `exclude_globs`, NIE väčší timeout): vylúč DDL/dáta (`migracie.ts`, `fonts/**`) z mutácie — overené `migration-vN.test.ts`. Logika #277 (ponuka-pdf/ponuka/dopyt*/datum) ostáva plne mutovaná.
- **Verzia #174 recovery:** 0.24.21-dev.1 sa omylom dostalo na main (chýbal clean-bump pred PR). Clean 0.24.21 už neprejde version-check (sort -V radí -dev.1 vyššie) → patch increment na čistú 0.24.22 (PR #284).
...
## #276 — Zákaznícky 3D vizuál pergoly (INTEGRÁCIA + wiring do /konfigurator) + #285 advance

- **Merge build-only lane** `worktree-agent-a1d44c196020cc076` (tip `efc6fd6`) do `dev` — bez konfliktu (package.json aj autopilot-log čisté). Priniesol #276 komponent `VizualPergolaZakaznik.svelte` + #285 showroom engine (HDRI IBL `static/hdri/kloofendal_puresky_1k.hdr` 1.4 MB, PBR hliník `metalness:0`, reálne PCF tiene, NeutralToneMapping, deprecation-free three r0.185). 18 súborov, +1806/−70, lane 2039 testov zelených.
- **Wiring (jediná nová implementácia, commit `a391f1f`):** 3D náhľad namontovaný do verejnej `/konfigurator/+page.svelte` ako predajný „hero" súhrnu. Lazy dynamic import (3D bundle až po submite), `viz` snapshot pri submite (konzistentný so zobrazeným súhrnom), `{#key}` na rozmeroch (teardown-safe remount), `zobrazOvladanie={false}` (form = jediný zdroj pravdy, drag-to-orbit ostáva). `typSkla3D(nazovSkla)` v `konfigurator.ts` (katalógový názov skla → vizuálny odtieň, jednotkovo testované). RAL kód pre 3D = form-state `farba` zachytený pri submite (`suhrn.farba` je len display label). E2E: desktop viz=mid + mobil 390x844 viz=low, 0 console, žiaden únik.
- **Money-safety:** verejná route — guard `konfigurator-money-safety.test.ts` prešiel; vizuál import graf dosiahnuteľný z komponentu neobsahuje money kód/katalóg/server (grep celého lane vizuál stromu = čisté; Vizual3D neimportuje Vizual3DPoster/ZaskleniaNavrhVykres — poster cez snippet).
- **Živá verifikácia** (`vite dev` + Playwright, `/konfigurator?viz=high`): render OK — `data-viz-ready=true`, `__VIZ_CONTEXTS===1`, canvas 694x434, RAL 9005 čierny rám + opál sklo z „4.4.2 mliečne", 0 aplikačných console chýb (len dokumentovaný GL-driver perf note, filtruje `collectConsole`), žiaden únik.
