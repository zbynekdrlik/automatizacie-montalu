<script lang="ts">
	...
	// Vypracoval (krátke meno/iniciály, napr. "D. Vozak") dostane menej miesta ako Dátum
	// ("D.M.RRRR HH:MM" môže mať až 16 znakov, napr. "31.12.2026 23:59") — 50/50 delenie
	// nechávalo Dátum orezaný clipPath-om (nález review #137).
	const leftHalfW = $derived(leftW * 0.36);
...
<g data-testid="title-block" font-family="inherit">
	<defs>
		<!-- ochrana proti pretečeniu dlhých hodnôt (reálne názvy/dátumy/OP čísla môžu
		     byť dlhšie než demo dáta) — KAŽDÁ hodnota sa ORAMUJE presne na svoju bunku,
		     nikdy nepretečie mimo rám pečiatky ani do susednej bunky (nález review #137:
		     pôvodne mala clipPath len NÁZOV/PROJEKT/ČÍSLO VÝKRESU, Dátum a ostatné nie). -->
		...
		>
		<clipPath id="tb-clip-mierka-{uid}"
			><rect x={leftW + cisloW} y={y2} width={rightW - cisloW} height={y3 - y2} /></clipPath
		>
		<clipPath id="tb-clip-revizia-{uid}"
			><rect x={leftW} y={y3} width={reviziaW} height={y4 - y3} /></clipPath
		>
		<clipPath id="tb-clip-varianta-{uid}"
			><rect x={leftW + reviziaW} y={y3} width={rightW - reviziaW} height={y4 - y3} /></clipPath
		>
		<clipPath id="tb-clip-vypracoval-{uid}"
			><rect x="0" y={y3} width={leftHalfW} height={height - y3} /></clipPath
		>
		<clipPath id="tb-clip-datum-{uid}"
			><rect x={leftHalfW} y={y3} width={leftW - leftHalfW} height={height - y3} /></clipPath
		>
	</defs>
	...
	<line x1={leftHalfW} y1={y3} x2={leftHalfW} y2={height} stroke="#0f172a" stroke-width="0.35" />
	...
	<g clip-path="url(#tb-clip-vypracoval-{uid})">
		<text x="2" y={height - 3} font-size="3.4" fill={VAL} data-testid="tb-vypracoval"
			>{vypracoval}</text
		>
	</g>
	<text x={leftHalfW + 2} y={y3 + 4} font-size="2.6" fill={LBL}>Dátum</text>
	<g clip-path="url(#tb-clip-datum-{uid})">
		<text x={leftHalfW + 2} y={height - 3} font-size="2.6" fill={VAL} data-testid="tb-datum"
			>{datum}</text
		>
	</g>
	...
	<g clip-path="url(#tb-clip-mierka-{uid})">
		<text
			x={leftW + cisloW + 2}
			y={y3 - 1.5}
			font-size="3.4"
	...
	<g clip-path="url(#tb-clip-revizia-{uid})">
	...
	<g clip-path="url(#tb-clip-varianta-{uid})">
