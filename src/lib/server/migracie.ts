// SQLite migrácie + seed dáta — extrahované z `db.ts` (#183), aby `db.ts`
// zostal len pripojenie + query API. `migrate()` berie `db`/`hashPassword` ako
// PARAMETRE (nie import z `./db`) — žiadny cyklický import medzi `db.ts` a
// TÝMTO súborom v ani jednom smere. Nulová zmena správania oproti pôvodnému
// `db.ts`: rovnaké funkcie, rovnaké poradie volaní, rovnaká `PRAGMA user_version`
...
export function migrate(db: Database.Database, hashPassword: (password: string) => string): void {
	...
	migrateOdooLeadColumns(db, bump); // v25→v26 (#278); extrahované do migracie-seed (large-file-split)

	if ((db.pragma('user_version', { simple: true }) as number) < 27) {
		// v26 → v27: ochrany proti dvojitému importu do Money (#294).
		//
		// (1) `odpis_imported` — APPEND-ONLY ledger MIMO mazateľného dedup kľúča. `releaseOdpis`
		//     aj kompenzácia mažú `odpis_log`, takže poistka proti re-importu nemôže žiť tam.
		//     `writeOdpis` zapíše `kind='import'` riadok AŽ PO úspešnom zápise súboru; keď identický
		//     obsah (tuple modul+zak_norm+op_norm+live + content_hash) už bol importovaný a nebol
		//     RE-autorizovaný (`kind='override'`), ďalší zápis sa zablokuje. Override je deliberátna
		//     auditovaná akcia na /odpisy (imports > overrides ⇒ blok; jeden override = jeden
		//     povolený re-import). NIKDY globálny hash — dve rôzne zákazky smú mať rovnaký obsah.
		// (2) `odpis_log.zak_norm`/`op_norm` — normalizovaný dedup kľúč (OP260286 ≡ 260286). RAW
		//     `UNIQUE(modul,zak,op,live)` ostáva pre atomicitu race-u; normalizácia je LOOKUP-based
		//     (žiadny UNIQUE na norm stĺpcoch — existujúce kolidujúce riadky id 46/47 by ho rozbili,
		//     a verdikt zakazuje deštruktívnu migráciu existujúcich riadkov → backfill raw copy,
		//     „stay as-is"; nové zápisy z `writeOdpis` idú už normalizované).
		// Money-NEUTRÁLNE (žiadny odpis sa migráciou nemení). Celé v `db.transaction` (vzor v24/v25):
		// ALTER/CREATE sú v SQLite transakčné → pád uprostred sa čisto prehrá, nikdy crash-loop.
		// `odpis_log` VŽDY existuje na reálnej v26 DB (vytvorené vo v1/v2), takže ALTER na nej vždy
		...
		db.transaction(() => {
			db.exec(`
				CREATE TABLE odpis_imported (
					...
					kind TEXT NOT NULL CHECK (kind IN ('import','override')),
	...
	migrateHrubkaTrieda(db, bump); // v36→v37 (#443)
	migrateMaterialRozvin(db, bump); // v37→v38 (#369)
	...
	migrateMaterialNakupSkladovaKarta(db, bump); // v44→v45 (#506)
	seedData(db);
