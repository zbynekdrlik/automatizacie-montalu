...
export interface PergolaNavrhVstup {
	...
	zvody: ZvodPozicia[];
	/** display text pre červenú poznámku "RAL: {ral}" (ako doteraz) — pri výbere
	 *  známeho odtieňa z `RAL_PALETA` sa nastaví automaticky na "{kod} {nazov}",
	 *  pri `ralKod === RAL_INY_KOD` ho vypĺňa používateľ voľne (#150) */
	ral: string;
	...
	varianta: string;
	vypracoval: string;
	/** režim výkresu (#150) — technický (čiernobiely, default) alebo farebný podľa
	 *  zvoleného RAL. Kóty/poznámky/raster/pečiatka sa NIKDY nefarbia — len konštrukcia. */
	rezimVykresu: PergolaVykresRezim;
	/** kód vybraného odtieňa z `RAL_PALETA`, `RAL_INY_KOD` ("iny") pre voľný text,
	 *  alebo '' keď nič nevybraté — `farbaKonstrukcie` má pre oba posledné prípady
	 *  čestný neutrálny fallback (#150). */
	ralKod: string;
}
