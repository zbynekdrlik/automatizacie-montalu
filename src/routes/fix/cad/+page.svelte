...
{#if step === 'form'}
	...
	<div class="card">
		<div class="sec">Fix z cadu · CAD nárez → Money odpis</div>
		...
		<form method="POST" action="?/spocitat">
			...
			<div class="field">
				<label style="display:flex;align-items:center;gap:8px;font-weight:400">
					...
					⏳ Čaká na materiál (odloží import do priečinka NA ODPIS/Fix)
				</label>
			</div>
...
{:else if step === 'nahlad' && v}
	<div class="card">
		...
		<p class="sub">
			<span class="badge">Fix · {v.totalBars} tyčí</span>
	...
	{#if v.longNotes.length}
		<div class="warn">
			<b>⚠ Dlhé profily (rez &gt; 7500 mm)</b> — riešené kombináciou tyčí. Over, že spoj vyjde na správnom
			mieste.
		</div>
	{/if}
	...
	<div class="card">
		<form method="POST" action="?/odoslat">
			...
			{#if v.kombinacie.length}
				<div class="sec noprint">Výber kombinácií tyčí</div>
			...
			<button class="btn noprint" type="submit" data-testid="odoslat">
				{data.live
					? vstup.caka
						? '⏳ Odoslať odpis (odloží sa do NA ODPIS/Fix)'
	...
	<!-- ceny materiálu (display-only) — LEN interní; b2b nikdy nedostane `form.ceny`. -->
	{#if ceny}
		<div class="noprint">
			<CenyTabulka {ceny} />
		</div>
...
{:else if step === 'hotovo' && v && form?.outcome}
	<div class="card">
		...
		<p class="sub"><span class="badge">Fix · {v.totalBars} tyčí</span></p>
	</div>
	...
	<div class="okmsg" data-testid="vysledok">
		...
		{:else if vstup.caka}
			⏳ Odložené — čaká na materiál. Súbor <b>{form.outcome.filename}</b> je v NA ODPIS/Fix, presuň do
	...
	<div class="card noprint">
		...
		<a class="btn secondary" href={resolve('/fix/cad')}>➕ Nový rozpis</a>
	</div>
...
{:else if step === 'duplikat'}
	...
	<div class="card noprint">
		<a class="btn secondary" href={resolve('/fix/cad')}>← Späť na formulár</a>
