...
async function odpisRiadky(page: Page): Promise<string[]> {
  ...
  return (await karta.locator(".row").allTextContents()).map((t) =>
    t.replace(/\s+/g, " ").trim(),
  );
}

test("klín: nič sa nepredvypĺňa z posuvu — všetky štyri kóty sú povinné", async ({
  page,
}) => {
...
test('klín prežije „← Späť a upraviť" (vrátane počtu kusov)', async ({
  page,
}) => {
  const errs = collectConsole(page);
  await loginAs(page);
...
test("viac posuvov: klín má len ten posuv, ktorý ho má zapnutý", async ({
  page,
}) => {
  const errs = collectConsole(page);
  await loginAs(page);
...
test("klín: obídená HTML5 validácia → serverová chyba, plán sa nespočíta", async ({
  page,
}) => {
  const errs = collectConsole(page);
  await loginAs(page);
